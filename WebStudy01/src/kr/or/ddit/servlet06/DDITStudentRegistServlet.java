...
//		vo.setHp(req.getParameter("hp"));
		...
		// 1. reflection을 몰라도 파라미터를 VO로 바인딩하려면??
		Map<String, String[]> parameterMap = req.getParameterMap();
		try {
			BeanUtils.populate(vo, parameterMap);
		} catch (IllegalAccessException | InvocationTargetException e) {
			// reflection 기술은 불확실성을 기반으로 하기 때문에 여러 예외에 대한 처리가 필요함.
			// 예외의 두 가지 종류(checked/unchecked)중
			// 예외에 대한 제어권이 servlet container로 전달된 후 500에러로 처리됨.
