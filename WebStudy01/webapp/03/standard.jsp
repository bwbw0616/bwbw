...
<pre>	
	...
	** JSP 토큰 구성 요소 : backend 방식 동작 코드
	...
	%> <!--  // 이 주석 표시를 해제하면 컴파일 에러 방식 -->
	2. Expression : <%="// 표현하기 위한 값이나 변수 혹은 식" %>
	3. Directive : &lt;%@ 3가지 지시자 중 하나의 지시자와 해당 지시자의 속성 및 값 %&gt;
	4. Declaration : <%! // 전역 변수나 전역 메소드가 선언
		String instanceVariable = "전역 변수의 값";
	%>
	5. Comment : <%--background방식 주석 --%>
		1) HTML, JavaScript, Css (FrontEnd comment)
		2) Java(single, multiline), JSP (BackEnd Comment) (보안에 유리함)
<!-- 		주석 -->
<script>
// 주석
</script>
	...
	** Servlet container의 역할
	1. 이미 등록되고 매핑 걸려있는 서블릿을 대상으로 최초의 요청이 발생하면,
		1) 해당 서블릿의 싱글톤 객체 생성.(두 번째 요청에서는 생략.)
	...
	** 참고
	언어의 종류
	1. 정적 타입 언어, 컴파일 방식 언어 -> 소스 작성 후 한 번의 번역을 통해, 전체 소스를 파싱하고 번역함.
		Java, C, C++
		int number;
			number = 2;
			<!-- number = "2";  이런건 정적 타입 언어에서 불가능 -->
	2. 동적 타입 언어, 인터프리팅 방식 언어, 스크립트 형태 언어-> 소스 작성 후 한 줄 한 줄 단위로 번역과 실행이 반복됨.
		JavaScript, Python
		var number;
			number = 2;
			number = "2";
